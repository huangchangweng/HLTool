...
+ (UIColor *)hl_fromHexValue:(NSUInteger)hex {
    ...
    return [UIColor hl_fromHexValue:hex alpha:fa];
}
...
+ (UIColor *)hl_colorWithString:(NSString *)string {
    ...
    if ([color hasPrefix:@"#"]) {
        ...
        if (color.length == 3) {
            ...
            return [UIColor hl_fromShortHexValue:hexRGB alpha:alpha];
        } else if (color.length == 6) {
            NSUInteger hexRGB = strtol(color.UTF8String, nil, 16);
            return [UIColor hl_fromHexValue:hexRGB alpha:alpha];
        }
        ...
        if (color.length == 8){
            NSUInteger hexRGB = strtol(color.UTF8String, nil, 16);
            return [UIColor hl_fromHexValue:hexRGB];
        } else if (color.length == 6) {
            NSUInteger hexRGB = strtol(color.UTF8String, nil, 16);
            return [UIColor hl_fromHexValue:hexRGB alpha:1.0f];
        }
    }
...
+ (UIColor *)hl_colorWithHex:(long)hex {
    ...
    int g = (hex >> 8)  & 0xFF;
    int b = hex & 0xFF;
    return [UIColor hl_colorWithIntegerRed:r green:g blue:b alpha:1.0];
}
